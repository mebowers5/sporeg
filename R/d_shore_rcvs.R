...
d_shore_rcvs <- function(km, study_site, land_barrier, epsg, sts_pts){
  ...
  grid_centroids <- grid_res(km, study_site, epsg, what = "centers") %>%  # Take the centroids of the grid
    sf::st_as_sf() %>% # Convert to sf object
    dplyr::mutate(d_shore = as.numeric(sf::st_distance(., land_barrier, by_element = FALSE))) # Find the distance to shore for each grid centroid

  # Correct coastal distances and renumber IDs
  grid_centroids <- grid_centroids %>%
    dplyr::mutate(d_shore = dplyr::if_else(sf::st_intersects(., land_barrier, sparse = F), 0, d_shore), # Grid centroids that instersect land have a shore distance of zero
  ...
  grid <- sf::st_intersection(grid, study_site) %>% # Clip the grid to the study site
    dplyr::mutate(count = lengths(sf::st_intersects(., sts_pts)), # Count the number of receivers in each grid cell
