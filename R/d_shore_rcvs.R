...
d_shore_rcvs <- function(km, study_site, land_barrier, epsg, sts_pts) {
  ...
  grid_centroids <- grid_res(km, study_site, epsg, what = "centers") |> # Take the centroids of the grid
    ...
    dplyr::mutate(
      ...
      d_shore = as.numeric(sf::st_distance(
        geometry,
        land_barrier,
        by_element = FALSE
      )),
      ...
      d_shore = dplyr::if_else(
        as.vector(
          sf::st_intersects(geometry, land_barrier, sparse = F),
        ),
        0,
        d_shore
      ), # Grid centroids that instersect land have a shore distance of zero
      gid2 = seq_along(geometry)
  ...
  sts_pts <- sf::st_set_agr(sts_pts, "constant")

  grid <- sf::st_intersection(grid, study_site) |> # Clip the grid to the study site
    dplyr::mutate(
      count = lengths(sf::st_intersects(geometry, sts_pts)), # Count the number of receivers in each grid cell
      ...
      den_rcs = (count / units::set_units(sf::st_area(geometry), km^2))
