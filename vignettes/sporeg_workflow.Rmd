...
Let's load the packages that we will need to run the steps in this vignette.
...
results <- lapply(results, dplyr::bind_rows, .id = 'resolution')

# Bind the bound resolution by iteration
results <- dplyr::bind_rows(results, .id = 'iteration')

head(results)
```

Hooray, we've run `sporeg` using 2 iterations! Deeper investigation requires many more than 2 iterations, but this can take a long time. To speed things up, let's load a model we've already run and work from that instead.

```{r}
#| label: import-previous-run
#| include: true
results <- sporeg::results |>
  lapply(dplyr::bind_rows, .id = 'resolution') |>
...
results <- new_df %>%
  group_by(res_name, gid) %>%
  summarise(avg_dif = mean(dif), se = plotrix::std.error(dif))
results
```
...
recon <- df_var %>%
  ...
  group_by(res_name) %>%
  summarise(
    max_30 = length(which(max_recon >= 30)) / max(iteration) * 100,
...
HS_100km_grid <- get_depth(HS_100km_grid, depth_data)
HS_50km_grid <- get_depth(HS_50km_grid, depth_data)
HS_25km_grid <- get_depth(HS_25km_grid, depth_data)
HS_10km_grid <- get_depth(HS_10km_grid, depth_data)
```
...
HS_100km_grid <- merge(
  (HS_100km_grid %>% as.data.frame() %>% dplyr::select(-geometry)),
  d_shore_rcvs(100, fo_study_site, fo_land_barrier, epsg, fo_sts_pts),
  by = "gid"
) %>%
  sf::st_as_sf()

HS_50km_grid <- merge(
  (HS_50km_grid %>% as.data.frame %>% dplyr::select(-geometry)),
  d_shore_rcvs(50, fo_study_site, fo_land_barrier, epsg, fo_sts_pts),
  by = "gid"
) %>%
  sf::st_as_sf()

HS_25km_grid <- merge(
  (HS_25km_grid %>% as.data.frame %>% dplyr::select(-geometry)),
  d_shore_rcvs(25, fo_study_site, fo_land_barrier, epsg, fo_sts_pts),
  by = "gid"
) %>%
  sf::st_as_sf()

HS_10km_grid <- merge(
  (HS_10km_grid %>% as.data.frame %>% dplyr::select(-geometry)),
  d_shore_rcvs(10, fo_study_site, fo_land_barrier, epsg, fo_sts_pts),
  by = "gid"
) %>%
  sf::st_as_sf()
```
...
grids <- sf::st_as_sf(dplyr::bind_rows(grids, .id = 'resolution')) %>%
  left_join(
    tibble(resolution = 1:4, res_name = c("100km", "50km", "25km", "10km")),
    by = "resolution"
  ) %>%
  mutate(
    res_name = ordered(res_name, levels = c("100km", "50km", "25km", "10km"))
...
results <- results %>%
  mutate(
    g_fit = ifelse(avg_dif >= -1 * 0.1 * anims & avg_dif <= 0.1 * anims, 1, 0)
  )

HS_100km_grid <- merge(
  results %>% dplyr::filter(res_name == "100km"),
  HS_100km_grid,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS_50km_grid <- merge(
  results %>% dplyr::filter(res_name == "50km"),
  HS_50km_grid,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS_25km_grid <- merge(
  results %>% dplyr::filter(res_name == "25km"),
  HS_25km_grid,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS_10km_grid <- merge(
  results %>% dplyr::filter(res_name == "10km"),
  HS_10km_grid,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS100km <- merge(
  (HS_100km_grid %>% as.data.frame() %>% dplyr::select(-x)),
  HS100km,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS50km <- merge(
  (HS_50km_grid %>% as.data.frame() %>% dplyr::select(-x)),
  HS50km,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS25km <- merge(
  (HS_25km_grid %>% as.data.frame() %>% dplyr::select(-x)),
  HS25km,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
HS10km <- merge(
  (HS_10km_grid %>% as.data.frame() %>% dplyr::select(-x)),
  HS10km,
  by = c("gid")
) %>%
  sf::st_as_sf()
...
vif.fit <- glm(
  formula = g_fit ~ den_rcs + p_a + mean_depth + d_shore,
  family = binomial(),
  data = HS_10km_grid
)
...
fit2.100km <- glm(
  formula = g_fit ~ p_a + mean_depth + d_shore,
  family = binomial(),
  data = HS_100km_grid
)
...
fit2.50km <- glm(
  formula = g_fit ~ den_rcs + p_a + mean_depth,
  family = binomial(),
  data = HS_50km_grid
)
...
demo_dets <- momentuHMM::crawlWrap(
  obsData = demo_dets,
  timeStep = "15 min",
  fixPar = c(NA, NA),
  theta = c(8, 0),
...
at_lines <- at_dly_locs %>%
  ...
  mutate(
    data = purrr::map(data, ~ mutate(.x, x = sf::st_sfc(x))),
