...
#> Ncells 5463597 291.8    9207830 491.8  9207830 491.8
#> Vcells 9671340  73.8   17824054 136.0 14786712 112.9
```
...
#> [[1]]
#>  [1] "doFuture"   "foreach"    "future"     "stplanr"    "forcats"    "stringr"    "purrr"      "readr"      "tidyr"     
#> [10] "tibble"     "ggplot2"    "tidyverse"  "sfnetworks" "pathroutr"  "lubridate"  "crawl"      "momentuHMM" "dplyr"     
#> [19] "glatos"     "sp"         "sf"         "sporeg"     "stats"      "graphics"   "grDevices"  "utils"      "datasets"  
#> [28] "methods"    "base"      
#> 
#> [[2]]
#>  [1] "doFuture"   "foreach"    "future"     "stplanr"    "forcats"    "stringr"    "purrr"      "readr"      "tidyr"     
...
#> Column `res_name` is not found.
#> Column `gid` is not found.
...
how_well_ct <- results %>%
  ...
  distinct()
#> Error in `mutate()`:
#> ℹ In argument: `res_name = ordered(res_name, levels = c("100km", "50km", "25km", "10km"))`.
#> Caused by error:
#> ! object 'res_name' not found
...
gc()
#>            used  (Mb) gc trigger   (Mb) max used   (Mb)
#> Ncells  7722622 412.5   22833067 1219.5 22833067 1219.5
#> Vcells 15532111 118.6   37529736  286.4 37529733  286.4
```
...
#> Error in `mutate()`:
#> ℹ In argument: `g_fit = ifelse(...)`.
#> Caused by error:
#> ! object 'avg_dif' not found

HS_100km_grid <- merge(results %>% dplyr::filter(res_name == "100km"), HS_100km_grid, by = c("gid")) %>%
  sf::st_as_sf() 
#> Error in (function (cond) : error in evaluating the argument 'x' in selecting a method for function 'merge': ℹ In argument: `res_name == "100km"`.
#> Caused by error:
#> ! object 'res_name' not found

HS_50km_grid <- merge(results %>% dplyr::filter(res_name == "50km"), HS_50km_grid, by = c("gid")) %>%
  sf::st_as_sf() 
#> Error in (function (cond) : error in evaluating the argument 'x' in selecting a method for function 'merge': ℹ In argument: `res_name == "50km"`.
#> Caused by error:
#> ! object 'res_name' not found

HS_25km_grid <- merge(results %>% dplyr::filter(res_name == "25km"), HS_25km_grid, by = c("gid")) %>%
  sf::st_as_sf() 
#> Error in (function (cond) : error in evaluating the argument 'x' in selecting a method for function 'merge': ℹ In argument: `res_name == "25km"`.
#> Caused by error:
#> ! object 'res_name' not found

HS_10km_grid <- merge(results %>% dplyr::filter(res_name == "10km"), HS_10km_grid, by = c("gid")) %>%
  sf::st_as_sf() 
#> Error in (function (cond) : error in evaluating the argument 'x' in selecting a method for function 'merge': ℹ In argument: `res_name == "10km"`.
#> Caused by error:
#> ! object 'res_name' not found
```
