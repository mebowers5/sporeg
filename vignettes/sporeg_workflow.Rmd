...
#> Ncells 1862652 99.5    3613974 193.1  3613974 193.1
#> Vcells 4826901 36.9   10212654  78.0 10211354  78.0
```
...
library(oceanmap)
#> Warning: package 'oceanmap' was built under R version 4.4.0
#> Loading required package: maps
#> Warning: package 'maps' was built under R version 4.4.0
#> Loading required package: mapdata
#> Warning: package 'mapdata' was built under R version 4.4.0
#> Loading required package: raster
#> Warning: package 'raster' was built under R version 4.4.0
#> 
...
#> Warning: package 'extrafont' was built under R version 4.4.0
#> Registering fonts with R
#> Loading required package: reshape2
#> Warning: package 'reshape2' was built under R version 4.4.0
#> Error: package or namespace load failed for 'oceanmap':
#>  Functions found when exporting methods from the namespace 'spam' which are not S4 generic: 'crossprod', 'tcrossprod'
library(car)
...
#> [[1]]
#>  [1] "doFuture"   "foreach"    "future"     "stplanr"    "forcats"    "stringr"    "purrr"      "readr"     
#>  [9] "tidyr"      "tibble"     "ggplot2"    "tidyverse"  "sfnetworks" "pathroutr"  "lubridate"  "crawl"     
#> [17] "momentuHMM" "dplyr"      "glatos"     "sp"         "sf"         "sporeg"     "stats"      "graphics"  
...
#> Warning in readChar(con, 5L, useBytes = TRUE): cannot open compressed file '', probable reason 'Invalid argument'
...
pow_stat <- df_var %>% 
  ...
  ungroup() %>%
  group_by(res_name)
#> Error in eval(expr, envir, enclos): object 'df_var' not found

res <- pow_stat %>% dplyr::filter(res_name == "100km")
#> Error in eval(expr, envir, enclos): object 'pow_stat' not found
pwr_100km <- powr(res, sig.level, power, delta)
#> Error in output$sd: object of type 'closure' is not subsettable
print("Grid cell resolution: 100 km x 100 km")
#> [1] "Grid cell resolution: 100 km x 100 km"
pwr_100km
#> Error in eval(expr, envir, enclos): object 'pwr_100km' not found
...
pwr_50km
#> Error in eval(expr, envir, enclos): object 'pwr_50km' not found
...
pwr_25km
#> Error in eval(expr, envir, enclos): object 'pwr_25km' not found
...
pwr_10km
#> Error in eval(expr, envir, enclos): object 'pwr_10km' not found
```
...
#> Error in eval(expr, envir, enclos): object 'pow_stat' not found
max_sd
#> Error in eval(expr, envir, enclos): object 'max_sd' not found
```
...
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'merge': object 'new_iters' not found

new_results <- new_iters_df %>%
  group_by(res_name, gid) %>%
  summarise(avg_dif = mean(dif),
            se = plotrix::std.error(dif)) 
#> Error in eval(expr, envir, enclos): object 'new_iters_df' not found
new_results
#> Error in eval(expr, envir, enclos): object 'new_results' not found
...
new_how_well
#> Error in eval(expr, envir, enclos): object 'new_how_well' not found
```
...
gc()
#>            used  (Mb) gc trigger  (Mb) max used  (Mb)
#> Ncells  5917178 316.1   14904267 796.0 14904267 796.0
#> Vcells 12412687  94.8   31289210 238.8 31289210 238.8
```
...
HS_100km_grid <- merge(results %>% dplyr::filter(res_name == "100km"), HS_100km_grid, by = c("gid")) %>%
  sf::st_as_sf() 
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'merge': object 'results' not found
...
#> Error in eval(predvars, data, env): object 'g_fit' not found

car::vif(dummy.fit)
#> Error in eval(expr, envir, enclos): object 'dummy.fit' not found
```
...
#> Error in eval(expr, envir, enclos): object 'fit2.100km' not found
...
car::Anova(fit3.50km)
#> Error in eval(expr, envir, enclos): object 'fit3.50km' not found
...
car::Anova(fit2.25km)
#> Error in eval(expr, envir, enclos): object 'fit2.25km' not found
...
car::Anova(fit1.10km)
#> Error in eval(expr, envir, enclos): object 'fit1.10km' not found
```
...
```r
summary(fit2.100km)
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'object' in selecting a method for function 'summary': object 'fit2.100km' not found
...
summary(fit3.50km)
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'object' in selecting a method for function 'summary': object 'fit3.50km' not found
...
summary(fit2.25km)
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'object' in selecting a method for function 'summary': object 'fit2.25km' not found
...
summary(fit1.10km)
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'object' in selecting a method for function 'summary': object 'fit1.10km' not found
...
#> Error in eval(expr, envir, enclos): object 'odds_100km' not found
```
...
rcv_dens <- data.table::rbindlist(rcv_dens, idcol = 'resolution') %>%
  left_join(tibble(resolution = 1:4, 
            ...
            by = "resolution") %>%
  mutate(res_name = ordered(res_name, levels = c("100km", "50km", "25km", "10km")))
#> Error in eval(expr, envir, enclos): object 'rcv_dens' not found
```
...
gd_fit_char <- lapply(res, gd_fts) %>%
  ...
  mutate(res_name = ordered(res_name, levels = c("100km", "50km", "25km", "10km")))
#> Error in `stopifnot()`:
#> ℹ In argument: `g_fit == 1`.
#> Caused by error:
#> ! object 'g_fit' not found
```
...
dif_depth <- lapply(res, dif_co) %>%
  ...
  mutate(res_name = ordered(res_name, levels = c("100km", "50km", "25km", "10km")))
#> Error in `dplyr::filter()`:
#> ℹ In argument: `mean_depth <= depth_limit`.
#> Caused by error:
#> ! argument "depth_limit" is missing, with no default
```
...
#> Warning in st_cast.sf(barrier, "POLYGON"): repeating attributes for all sub-geometries for which they may not be
#> constant
buffer <- 650
